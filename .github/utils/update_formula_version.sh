...
update_formula_version() {
    ...
    if [[ "$VERSION" == *.*.* && "$VERSION" != *.*.*-* && ! -f $formula_version_file ]]; then
    ...
    formula_version=$( cat $formula_version_file | grep version | awk 'NR==1{print $2}' | sed 's/\"//g' )
    # update formula kbcli.rb version
    echo "update formula_version:"$formula_version" to release_version:"$VERSION
    update_file $formula_version $VERSION $formula_version_file
    url_sha256=""
    line_num=0
    while read -r readline; do
        ...
        if [[ "$readline" == *"url"* ]];then
            ...
            update_file_with_line_num $sha256 $url_sha256 $formula_version_file $line_num
            url_sha256=""
        fi
    done < $formula_version_file
}
